# API設計書

> **出力先**: `docs/api/API設計書/`
> **ファイル名**: `API-CAT-003_カテゴリー作成.md`

## 1. 文書情報

| 項目 | 内容 |
| :--- | :--- |
| **API ID** | API-CAT-003 |
| **API名** | カテゴリー作成 |
| **カテゴリ** | カテゴリー管理 |
| **バージョン** | 1.0.0 |
| **作成日** | 2025-12-10 |
| **作成者** | API設計エージェント |
| **最終更新日** | 2025-12-10 |
| **最終更新者** | API設計エージェント |

---

## 2. API概要

| 項目 | 内容 |
| :--- | :--- |
| **エンドポイント** | `/categories` |
| **HTTPメソッド** | `POST` |
| **機能概要** | 新規カテゴリーをデータベースに登録する。バリデーションを実施し、成功時は一覧画面へリダイレクトする。 |
| **処理種別** | `データ登録` |
| **トランザクション** | `あり` |
| **認証** | `不要` |
| **必要権限** | `なし` |

### 使用場面

- カテゴリー登録画面での登録ボタン押下時

---

## 3. リクエスト仕様

### 3.1 パスパラメータ

なし

### 3.2 クエリパラメータ

なし

### 3.3 リクエストボディ

**Content-Type**: `application/x-www-form-urlencoded` (フォーム送信)

**フィールド定義**:

| フィールド名 | 型 | 必須 | 説明 | 制約 |
| :--- | :--- | :--- | :--- | :--- |
| `name` | `String` | ✔ | カテゴリー名 | 最大50文字、空白のみ不可 |
| `description` | `String` | - | 説明 | 最大200文字 |
| `color` | `String` | ✔ | カラーコード | 7文字（#RRGGBB形式） |
| `displayOrder` | `Integer` | ✔ | 表示順 | 0以上の整数 |

### 3.4 バリデーションルール

| 項目 | バリデーション内容 | エラーメッセージ |
| :--- | :--- | :--- |
| `name` | 必須、1～50文字、空白のみ不可 | "カテゴリー名を入力してください" |
| `name` | 最大50文字 | "カテゴリー名は50文字以内で入力してください" |
| `description` | 最大200文字 | "説明は200文字以内で入力してください" |
| `color` | 必須 | "カラーコードを入力してください" |
| `color` | #RRGGBB形式（正規表現: ^#[0-9A-Fa-f]{6}$） | "カラーコードの形式が不正です（例: #FF5733）" |
| `displayOrder` | 必須 | "表示順を入力してください" |
| `displayOrder` | 0以上の整数 | "表示順は0以上の整数を入力してください" |

**バリデーションアノテーション**:
- `@NotBlank`: name
- `@Size(max=50)`: name
- `@Size(max=200)`: description
- `@NotBlank`: color
- `@Pattern(regexp="^#[0-9A-Fa-f]{6}$")`: color
- `@NotNull`: displayOrder
- `@Min(0)`: displayOrder

---

## 4. レスポンス仕様

### 4.1 正常時レスポンス

* **HTTPステータスコード**: `302 Found` (リダイレクト)
* **リダイレクト先**: `/categories` (カテゴリー一覧画面)
* **フラッシュ属性**: `successMessage = "カテゴリーを登録しました"`

### 4.2 バリデーションエラー時レスポンス

* **HTTPステータスコード**: `200 OK`
* **Content-Type**: `text/html` (Thymeleafテンプレート)
* **レンダリングされるビュー**: `category/form.html`

**モデル属性**:

| 属性名 | 型 | 必須 | 説明 |
| :--- | :--- | :--- | :--- |
| `categoryForm` | `CategoryForm` | ✔ | 入力済みデータを保持したフォーム |
| `org.springframework.validation.BindingResult.categoryForm` | `BindingResult` | ✔ | バリデーションエラー情報 |

---

## 5. 処理詳細

### 5.1 処理ステップ

| ステップ | 処理内容 | 備考 |
| :--- | :--- | :--- |
| 1 | リクエストボディから CategoryForm を取得 | Spring が自動的にバインド |
| 2 | @Valid によりバリデーション実施 | - |
| 3 | バリデーションエラーがある場合、ステップ8へ | - |
| 4 | CategoryService.create(categoryForm) を呼び出し | トランザクション開始 |
| 5 | CategoryConverter でCategoryFormをCategoryエンティティに変換 | - |
| 6 | CategoryMapper.insert(category) でデータベースに登録（SQL1） | - |
| 7 | トランザクションコミット、フラッシュ属性に成功メッセージを設定し、一覧画面へリダイレクト | 正常終了 |
| 8 | ビュー `category/form.html` を再表示 | エラー終了 |

### 5.2 使用サービス

| サービスクラス | メソッド | 役割 |
| :--- | :--- | :--- |
| `CategoryService` | `create(categoryForm)` | カテゴリーを登録 |

### 5.3 使用テーブル

| テーブル名 | 物理名 | 操作 | 用途 |
| :--- | :--- | :--- | :--- |
| カテゴリー | `task_categories` | `INSERT` | カテゴリー情報の登録 |

### 5.4 SQL詳細

#### SQL1: カテゴリー登録

**目的**: 新規カテゴリーをデータベースに登録する

**Mapper**: `CategoryMapper.insert`

```sql
INSERT INTO task_categories (name, description, color, display_order, created_at, updated_at)
VALUES (#{name}, #{description}, #{color}, #{displayOrder}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
```

**パラメータ**:

| パラメータ名 | 型 | 説明 | 例 |
| :--- | :--- | :--- | :--- |
| `#{name}` | `String` | カテゴリー名 | `"仕事"` |
| `#{description}` | `String` | 説明 | `"仕事関連のタスク"` |
| `#{color}` | `String` | カラーコード | `"#007bff"` |
| `#{displayOrder}` | `Integer` | 表示順 | `1` |

**自動生成**:
- `id`: AUTO_INCREMENT
- `created_at`: CURRENT_TIMESTAMP
- `updated_at`: CURRENT_TIMESTAMP

---

## 6. ビジネスロジック

### 6.1 データ変換ロジック

**CategoryForm → Category エンティティ変換**:
- CategoryConverter (MapStruct) を使用
- descriptionがnullの場合、そのままnullとして保存される
- colorはデフォルト値として`#6c757d`を設定可能

---

## 7. トランザクション制御

| 項目 | 内容 |
| :--- | :--- |
| **トランザクション境界** | CategoryService.create() メソッド全体 |
| **コミット条件** | カテゴリー登録SQL正常実行時 |
| **ロールバック条件** | SQL実行エラー、データベース制約違反時 |
| **トランザクション管理** | Spring の @Transactional アノテーション |

---

## 8. エラーハンドリング

| エラー条件 | HTTPステータス | エラーコード | 処理内容 |
| :--- | :--- | :--- | :--- |
| バリデーションエラー | 200 | - | フォーム再表示、エラーメッセージ表示 |
| 重複カテゴリー名 | 200 | - | フォーム再表示、"同じカテゴリー名が既に存在します"エラー表示 |
| DB制約違反 | 500 | INTERNAL_SERVER_ERROR | 500エラー画面（SCR-CMN-002）を表示、ログ出力 |
| DB接続エラー | 500 | INTERNAL_SERVER_ERROR | 500エラー画面（SCR-CMN-002）を表示、ログ出力 |

---

## 9. 外部連携

該当なし

---

## 10. 関連API

| API名 | エンドポイント | 関係性 |
| :--- | :--- | :--- |
| カテゴリー登録フォーム表示 | `GET /categories/new` | フォーム表示元 |
| カテゴリー一覧取得 | `GET /categories` | 登録成功時のリダイレクト先 |

---

## 11. テストケース

### 11.1 正常系

| No. | テストケース | 入力値 | 期待結果 |
| :--- | :--- | :--- | :--- |
| 1 | 全項目指定で登録 | name="仕事", description="仕事関連", color="#007bff", displayOrder=1 | 302リダイレクト、成功メッセージ表示 |
| 2 | 必須項目のみで登録 | name="個人", color="#28a745", displayOrder=2 | 302リダイレクト、descriptionはnull |

### 11.2 異常系

| No. | テストケース | 入力値 | 期待結果 |
| :--- | :--- | :--- | :--- |
| 1 | カテゴリー名未入力 | name=null | 200、"カテゴリー名を入力してください"エラー表示 |
| 2 | カテゴリー名空白のみ | name="   " | 200、"カテゴリー名を入力してください"エラー表示 |
| 3 | カラーコード未入力 | color=null | 200、"カラーコードを入力してください"エラー表示 |
| 4 | カラーコード形式不正 | color="007bff"（#なし） | 200、"カラーコードの形式が不正です"エラー表示 |
| 5 | 表示順未入力 | displayOrder=null | 200、"表示順を入力してください"エラー表示 |
| 6 | 表示順負の数 | displayOrder=-1 | 200、"表示順は0以上の整数を入力してください"エラー表示 |

### 11.3 境界値

| No. | テストケース | 入力値 | 期待結果 |
| :--- | :--- | :--- | :--- |
| 1 | カテゴリー名最大文字数（50文字） | name="a" × 50 | 302リダイレクト、正常登録 |
| 2 | カテゴリー名文字数超過（51文字） | name="a" × 51 | 200、"カテゴリー名は50文字以内で入力してください"エラー表示 |
| 3 | 説明最大文字数（200文字） | description="a" × 200 | 302リダイレクト、正常登録 |
| 4 | 説明文字数超過（201文字） | description="a" × 201 | 200、"説明は200文字以内で入力してください"エラー表示 |
| 5 | 表示順=0 | displayOrder=0 | 302リダイレクト、正常登録 |

---

## 12. 備考

- Spring の @Valid アノテーションによりサーバーサイドバリデーションを実施
- バリデーションエラー時は入力済みデータを保持してフォームを再表示
- created_at と updated_at は自動設定（CURRENT_TIMESTAMP）
- カラーコードのデフォルト値は`#6c757d`（グレー）
- MyBatisマッパーは `CategoryMapper` (カスタムマッパー) を使用
- useGeneratedKeys=true により登録後の自動採番IDを取得可能

---

## 13. 変更履歴

| バージョン | 変更日 | 変更者 | 変更内容 |
| :--- | :--- | :--- | :--- |
| 1.0.0 | 2025-12-10 | API設計エージェント | 新規作成 |
